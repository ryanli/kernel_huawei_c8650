...
static int mcsdl_enter_download_mode(void)
{
	...
	ret = gpio_direction_output(reset_pin, 1);			/* CE set high */
...
firmware_find_device: 
	...
	if (buf[0] == '2') {
		...
		ret1 = gpio_tlmm_config(GPIO_CFG(TS_INT_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
static int melfas_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->irq) 
    {
        ...
        {
			MELFAS_DEBUG("Failed to request IRQ!\n");
		}
	}
