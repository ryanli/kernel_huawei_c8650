...
//delete two macro,move to board-7x27.c
...
static int cypress_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct i2c_msg msgz[1];
	struct touch_hw_platform_data *touch_pdata;
	int touch_reset_pin = 0xFF;

	/*delete four lines*/
	msgz[0].addr = client->addr;
	msgz[0].flags = 0;
	msgz[0].len = 12;
	msgz[0].buf = buf;
    ...
    if(ret)
    {
        pr_err(KERN_ERR "%s: the touch driver has detected! \n", __func__);
        return -ENODEV;
    }
    else
    {
        pr_err(KERN_ERR "%s: it's the first touch driver! \n", __func__);
    }
	...
	{
		touch_reset_pin = touch_pdata->get_touch_reset_pin();
		gpio_request(touch_reset_pin, CYPRESS_I2C_NAME);
		ret = gpio_direction_output(touch_reset_pin, 1);
		mdelay(10);
		ret = gpio_direction_output(touch_reset_pin, 0);
		mdelay(100);	
	}
	...
	/*the size of M650 LCD is 320*240(x*y) */
	ts->x_max = 320;
	ts->y_max = 240;
	...
	if (ret < 0) 
	{
		pr_err("%s: gpio_tlmm_config(%#x)=%d\n", __func__, irq_to_gpio(client->irq), ret);
		ret = -EIO;
		goto err_key_input_register_device_failed; 
	}
	...
	if (client->irq) 
	{
		pr_err("%s: client:0x%x\n", __func__, irq_to_gpio(client->irq));
		ret = request_irq(client->irq, cypress_ts_irq_handler, IRQF_TRIGGER_FALLING, client->name, ts);		
		if (ret == 0)
			ts->use_irq = 1;
		else
...
static int cypress_ts_resume(struct i2c_client *client)
{
	...
	{
		/*QC isn't support some API */
		gpio_tlmm_config(GPIO_CFG(irq_to_gpio(client->irq), 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_direction_output(irq_to_gpio(client->irq), 0);
		msleep(2);
		gpio_direction_output(irq_to_gpio(client->irq), 1);
		msleep(2);
		gpio_direction_output(irq_to_gpio(client->irq), 0);
		gpio_tlmm_config(GPIO_CFG(irq_to_gpio(client->irq), 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
